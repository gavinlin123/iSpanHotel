<template>
      ...
      >
        <template slot-scope="scope">
          <el-popover placement="top-start" title trigger="click">
            <img :src="'data:image/png;base64,'+scope.row.roomPic" alt style="width: 300" />
            <img slot="reference" :src="'data:image/png;base64,'+scope.row.roomPic" style="width: 50px;height: 50px" />
          </el-popover>
        </template>
      </el-table-column>
  ...
  <!-- 分頁 
    ...
    <!-- 新增表單 -->
    <el-dialog title="房型資料" :visible.sync="dialogFormVisible" width="50%">
      <el-form label-width="80px" size="small">
        <el-form-item label="房型名稱">
          <el-input v-model="addForm.roomType" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="入住人數">
          <el-input v-model="addForm.roomPerson" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="定價">
          <el-input v-model="addForm.roomPrice" autocomplete="off"></el-input>
        </el-form-item>
        <!-- ============================================================== -->
    ...
    <el-form :model="editForm" label-width="80px" size="small" > 
      ...
      <el-form-item label="入住人數">
        <el-input v-model="editForm.roomPerson" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="定價">
        <el-input v-model="editForm.roomPrice" autocomplete="off"></el-input>
      </el-form-item>
      <!-- ============================================================== -->
      <el-form-item label="照片">
      <!-- 修改照片 -->
...
export default {
  ...
  methods: {
      ...
      if (bool) {
        ...
        if (this.inputVal) {
          this.tableData.filter((item) => {
            // 房型名稱模糊查詢
            if (item.roomType.indexOf(this.inputVal) !== -1) {
    ...
    add() {
      ...
      formData.append("roomPic", this.fileList[0].raw);
      formData.append("roomPrice", this.addForm.roomPrice);
      formData.append("roomType", this.addForm.roomType);
      formData.append("roomPerson", this.addForm.roomPerson);
      
      this.$http.post("/roomtype/", formData,
      ).then((res) => {
        if (res) {
    ...
    edit() {
      ...
      formData.append("roomPic", this.fileList[0].raw);
      formData.append("roomPrice", this.editForm.roomPrice);
      formData.append("roomType", this.editForm.roomType);
      formData.append("roomPerson", this.editForm.roomPerson);
      this.$http.put(`/roomtype/` + this.editForm.id, formData
        ).then((res) => {
        if (res) {
    ...
    // 刪除按鈕=>透過id刪除
    del(id) {
      this.$http.delete("/roomtype/" + id).then((res) => {
        if (res) {
